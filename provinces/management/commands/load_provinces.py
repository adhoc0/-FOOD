...
# İl verisi için tip tanımlaması
    ...
    def handle(self, *args, **kwargs):
        ...
        for data in provinces_data:
            region, _ = Region.objects.get_or_create(
                name=data["region"],
                defaults={
                    "slug": region_slugs.get(data["region"], "region"),
                    "is_active": True,
                },
            )
            # Create simple turkish slug for province
            turkish_map = str.maketrans("ığüşöçİĞÜŞÖÇ ", "igusocigusoc-")
            province_slug = data["name"].lower().translate(turkish_map).replace("--", "-")
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Başarıyla {created_count} yeni il eklendi. Toplam 81 il mevcut.")
        )
